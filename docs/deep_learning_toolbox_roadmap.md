...
- **Status legend**: ✅ shipped · 🟡 partial · 🔵 not started. **T1 + T2 (the
  ...
  **HDL Tier-H1 + H2 ✅, H3 partial 🟡**: H1 ships `dlquantize`/`dlqscale`
  ...
  green).  Issue #75 closed.  **H4 LSTM-on-FPGA ✅** — two fixtures
  (`dlhdl_rnn_cell.m`, `dlhdl_lstm_cell.m`) demonstrate the recurrent
  kernel compiles to bit-accurate fi SV with hardsigmoid/hardtanh PWL
  activations on each gate.  cocotb sweep now 43/43.  Multi-timestep
  recurrence with persistent register state remains the follow-on
  slice.  **T5.7 Neural ODE + T6.2
...
| H4 | **LSTM-on-FPGA ✅** | Recurrent fi-typed cell forwards compiled to fi SV via the `precise_fi` opt-in.  Two shipped fixtures: `dlhdl_rnn_cell.m` (simple cell `h_new = hardtanh(Wx*x + Wh*h_prev + b)`) and `dlhdl_lstm_cell.m` (full LSTM with `hardsigmoid` i/f/o gates + `hardtanh` g gate + cell-state update + final hidden activation; 4 gate matmul-accumulate + 6 PWL muxes).  Both DUTs are combinational — caller threads `h_prev`/`c_prev` across timesteps externally.  cocotb bit-accuracy 100/100 on each.  Multi-timestep recurrence with persistent register state remains the documented follow-on slice (the FIR pipelined example shows the persistent-shift-register pattern). | H2 + precise_fi |
