...
| 1.4 | **`gpuArray` classdef + carrier** | Handle classdef with `Underlying` (host shape/dtype), `Device` (which backend), `DevicePtr` (opaque `void*`), `Stream` (per-device queue). `gpuArray(X)` → upload + return handle; `gather(g)` → device→host copy + return matrix; `existsOnGPU(g)` / `classUnderlying(g)` / `size`/`numel`/`isa` overloads. The classdef carrier reuses `matlab_obj_new`. **Status (#335 Tier A): `gpuArray` is now a *routable device-resident carrier* — a magic-tagged runtime descriptor (`MATLAB_GPU_MAGIC`, `struct matlab_gpu` in `runtime_internal.h`) wrapping the host matrix, NOT a classdef object (matlabc doesn't route free-fn/`size`/`gather` to methods, and an object doesn't survive the REPL workspace; the tag, living in the descriptor, does). `matlab_gpuArray_ctor` wraps, `matlab_gather` unwraps. Each op on the proving surface — `mtimes`, the element-wise `_mm`/`_ms`/`_sm` family, reductions (`sum`/`sum_all`), and the `size`/`numel`/`length`/`disp`/scalar inspectors — sniffs the tag and dispatches: device backend (Tier C) or host CPU fallback (Tier A, host-correct, results re-tagged so chains stay device-resident until `gather`). Supersedes the #333 identity builtin while preserving its host correctness. Uniform across AOT + JIT/REPL (runtime-level), round-trips the REPL workspace. Gated `test/Run/gpuarray_device_dispatch.m`. **Tier C (mtimes) landed:** with `-DMATLAB_LLVM_GPU_CUDA=ON` and `MATLAB_GPU_TARGET=auto`, an in-process gpuArray `Ag*Bg` now escalates to **cuBLAS Dgemm on the device** (`matlab_gpu_gemm`, N ≥ threshold) — verified on the RTX 5060 (≈76% GPU util, machine-precision result; `MATLAB_GPU_TRACE=1` shows the dispatch), host CPU fallback otherwise; gated by `test/Run/run_gpu_cuda_validation.sh`. Remaining Tier C: device dispatch for the element-wise/reduction surface (NVRTC kernels) + keeping data resident across chained ops. Tier B extends the dispatch surface. (Earlier #333 note: the original object carrier shipped without working operator/`gather`/`size` overloads, so `Ag*Bg` ran `matlab_matmul` on the object pointer and returned empty; identity restored host correctness but left no device hook — this change adds the hook.)** | `runtime_internal.h` `matlab_gpu`; `runtime/toolbox/gpu/runtime_gpu_helpers.cpp` |
