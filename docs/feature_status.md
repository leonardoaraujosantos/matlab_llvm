...
| Filter design — IIR lowpass + frequency response (Tier-1 §2.1) — `butter(n, Wn)`, `cheby1(n, Rp, Wn)`, `freqz(b, a, N)` (single + 2-return forms) | ✅ shipped (lowpass scope) | Bilinear-transform design from analog Butterworth / Chebyshev I prototypes; n complex poles → conjugate pairs after pre-warp + bilinear → real `(b, a)` of length n+1, normalized to unit DC gain. `freqz` evaluates `H(e^{jw})` at N equally spaced points on `[0, π)`; complex-magnitude works on the C / C++ / LLVM / Python lanes; TS lane drops the imaginary part (`NDArray` has no native complex type — same friction as `roots` and `fft_c` on TS, gated with `.skip-emit-typescript`). C / C++ / LLVM / Python lanes byte-identical (with `.stdout-python` override for numpy bracket repr). Gating: `test/Run/sig_iir.m` (4-lane) + 5 direct C unit tests in `test/Runtime/test_signal.c` (Butterworth unit DC gain + numerator symmetry + `freqz(z=1) = 1`, deep-stopband attenuation `< 0.01`, Chebyshev I passband ripple `> 1`). |
| Filter design — IIR follow-on: `cheby2`, `ellip`, `besself`, analog prototypes (`buttap`, `cheb1ap`, `cheb2ap`, `ellipap`, `besselap`), `bilinear`, order helpers (`buttord`, `cheb1ord`, `cheb2ord`, `ellipord`), high/band/stop variants of `butter`/`cheby1`, analog `freqs`, form conversions (`tf2zp`, `zp2tf`, `tf2sos`, `sos2tf`) | 🔵 | Follow-on slices on top of the §2.1 lowpass core. cheby2 needs separate j-axis zero handling; ellip needs Jacobi elliptic functions. |
