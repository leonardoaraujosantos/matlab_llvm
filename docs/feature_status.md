...
| Filter design — IIR lowpass + frequency response (Tier-1 §2.1) — `butter(n, Wn)`, `cheby1(n, Rp, Wn)`, `cheby2(n, Rs, Wn)`, `freqz(b, a, N)` (single + 2-return forms) | ✅ shipped (lowpass scope) | Bilinear-transform design from analog Butterworth / Chebyshev I / Chebyshev II prototypes; n complex poles → conjugate pairs after pre-warp + bilinear → real `(b, a)` of length n+1, normalized to unit DC gain. `cheby2` has finite j-axis zeros (n or n-1 depending on parity) handled by a generalized `lowpass_from_analog_pz_` helper. `freqz` evaluates `H(e^{jw})` at N equally spaced points on `[0, π)`; complex-magnitude works on the C / C++ / LLVM / Python lanes; TS lane drops the imaginary part (`NDArray` has no native complex type — same friction as `roots` and `fft_c` on TS, gated with `.skip-emit-typescript`). Gating: `test/Run/sig_iir.m` + `sig_iir_more.m` (4-lane each) + 5 direct C unit tests. |
| Order-selection helpers — `[n, Wn] = buttord(Wp, Ws, Rp, Rs)`, `[n, Wn] = cheb1ord(Wp, Ws, Rp, Rs)` | ✅ shipped | Lowpass scope. Butterworth uses `log10((10^(Rs/10)−1)/(10^(Rp/10)−1)) / (2 log10(Wsa/Wpa))`; Chebyshev I uses `acosh(sqrt(...)) / acosh(Wsa/Wpa)` with `Wn = Wp`. Multi-return splits via `matlab_<name>_n` / `_Wn` (eig precedent); single-LHS form returns `n`. |
| Filter design — IIR open: `ellip`, `besself`, analog prototypes (`buttap`, `cheb1ap`, `cheb2ap`, `ellipap`, `besselap`), standalone `bilinear`, `cheb2ord`/`ellipord`, high/band/stop variants of `butter`/`cheby1`/`cheby2`, analog `freqs`, form conversions (`tf2zp`, `zp2tf`, `tf2sos`, `sos2tf`) | 🔵 | Follow-on. `ellip` needs Jacobi elliptic functions; `besself` is continuous-time only; band variants need analog frequency transformations. |
...
| Filter implementation (Tier-1 §2.5) — `filtfilt(b, a, x)`, `sosfilt(sos, x)`, `impz(b, a, N)`, `stepz(b, a, N)`, `grpdelay(b, a, N)` | ✅ shipped | Internal `filter_flat_` helper (direct-form-II transposed) drives all five entries. `filtfilt` uses reflection padding of length `3·(L−1)` clamped to `nx-1` and zero initial conditions — small edge transient (~1% on a length-10 palindrome through `butter(3, 0.4)`). The Gustafsson 1996 initial-condition trick for true zero-edge-transient is a follow-on. `sosfilt` cascades L × 6 `[b0 b1 b2 a0 a1 a2]` rows. `impz`/`stepz` drive `filter_flat_` with `[1; 0; …]` / `ones(N, 1)`. `grpdelay` evaluates `H(e^{jω})` at the freqz grid and at `ω + dω` with one-step phase unwrapping in `(−π, π]`. Gating: `test/Run/sig_filt.m` (4-lane). |
| Filter implementation — open: `phasez`, `zerophase`, Gustafsson initial conditions for `filtfilt` | 🔵 | §2.5 follow-on. |
...
| Pulse / waveform measurements (Tier-3 §4.3) — `findpeaks` (1- and 2-return), scalar reductions `rms` / `peak2peak` / `peak2rms` / `rssq`, signal cleanup `medfilt1` / `hampel` / `envelope`, pulse statistics `midcross` / `risetime` / `falltime` / `dutycycle` | ✅ shipped (core surface) | findpeaks uses strict-monotonic local-maximum definition (no plateaus, no endpoints); MinPeak* options deferred. medfilt1 has zero-padded edges with odd-N coercion. hampel uses 3·1.4826·MAD threshold. envelope is peak-interpolation linear between local maxima. Pulse statistics auto-detect state levels from min/max with default 10/50/90% reference percentages. **Open**: MinPeakHeight/Distance/Prominence options for findpeaks; slewrate, pulseperiod, pulsewidth, overshoot, undershoot, settlingtime, statelevels (histogram-based). |
