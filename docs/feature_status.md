...
| **§3.1 minimum-viable `tf` (slice 1)** | ✅ shipped | `runtime/cst_classdefs.m` now provides `tf(num, den)` with `obj.Numerator` / `obj.Denominator` read access. Several compiler fixes landed alongside: (a) `LowerUserCalls.cpp` skips tensor-typed call-site refinement / monomorphization for class methods (gated on the `matlab.class_name` op attr) so post-LowerTensorOps ptr-typed call operands match the constructor signature; non-class user functions keep per-shape clones; (b) `LowerTensorOps.cpp` field-set dispatch auto-promotes `_set_f64` to `_set_mat` when the value operand turns out to be ptr after lowering-time settled, and `fieldNameAddr` accepts `LLVM::AddressOfOp` (not just `matlab.const_char`) so a second sweep can resolve the field-name pointer; (c) `Lowering.cpp` field-read dispatch routes to `matlab_obj_get_mat` for instances of the CST prelude classes (`tf`/`ss`/`zpk`/`pid`/`frd`) when Sema couldn't infer a concrete type — `matlab_struct_get_mat` already auto-boxes f64-stored fields back to 1×1, so the dispatch is a strict superset for these classes. Run-test `ctrl_tf_basic.m` (LLVM lane). **Open**: tf-vs-tf operator overloads, scalar mixing (`s + 2`), `tf('s')` builder — see `control_toolbox_roadmap.md` §12 "Still 🔵". |
