...
| `fi` (Fixed-Point Designer) | 🟡 | Phases 1–3: scalar `fi(value, signed, WL, FL)` constructors with literal-fold, `+ - *`, `(:)` type-preserving assignment, `Saturate` overflow, `Floor`/`Nearest` rounding, sub-native WL (e.g. WL=12 in i16 lane), implicit `fi + double` promotion, `int(n)` / `storedInteger(n)` / `double(n)`, `bin/hex/dec` display, **fi arrays** (`fi(zeros(1,N),...)`, indexing, slicing, vector concat, `sum`/`mean`), **persistent storage** of fi arrays. Gating test: FIR filter in `test/Run/fi_filter.m`. Storage = native `int8/16/32/64`. `numerictype` / `fimath` objects, slope/bias scaling, function-internal fi typing across user calls are deferred — see [`docs/emit_fixed_point.md`](emit_fixed_point.md). |
