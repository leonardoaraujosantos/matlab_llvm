...
- [`signal_toolbox_roadmap.md`](signal_toolbox_roadmap.md): tiered plan for Signal Processing Toolbox compatibility. **Tier-1 + Tier-2 bulk + Tier-3 §4.1/§4.2/§4.3/§4.4 are closed**. Tier-1: windows (17), polynomial helpers, IIR + order helpers, FIR, filter response, filter application. Tier-2: transforms, spectral, LP + parametric PSD, time-frequency. Tier-3: real multirate (`upfirdn`/`decimate`/`interp`/`resample` — replaces toy stubs), waveform generators (`chirp`/`sawtooth`/`square`/`gauspuls`/`rectpuls`/`tripuls`/`sinc`), alignment helpers (`xcov`/`finddelay`/`dtw`), pulse measurements (§4.3 surface). **Open follow-ons**: `ellip`/`besself`/band variants/analog prototypes (§2.1); `fir2`/`firls`/`firpm` (§2.2); Gustafsson IC for `filtfilt` (§2.5); `dpss`/`pmtm`/`czt`/`stft`/`istft`/subspace methods (Tier-2 follow-on); chirp non-linear methods (§4.2); MinPeak* options + slewrate/pulseperiod/pulsewidth (§4.3); `alignsignals`/`gccphat`/xcorr scaling-options (§4.4); polyphase decomposition (§4.1); vibration analysis (§4.6); `digitalFilter` system object (§5.1); wavelets (§5.4). Apps, GUIs, deep-learning, MATLAB Coder, Python coexecution explicitly carved out
