...
- [`signal_toolbox_roadmap.md`](signal_toolbox_roadmap.md): tiered plan for Signal Processing Toolbox compatibility. **Tier-1 lowpass design loop is closed**: windows tail (17 windows), polynomial helpers (`roots`/`poly`/`polyder`/`polyint`/`residue` distinct-pole), IIR design (`butter`/`cheby1`/`cheby2`) with order helpers (`buttord`/`cheb1ord`), FIR design (`fir1`/`sgolay`/`sgolayfilt`), filter response (`freqz`/`impz`/`stepz`/`grpdelay`), zero-phase + biquad-cascade application (`filtfilt`/`sosfilt`). **Open**: `ellip`/`besself`, high/band/stop variants, analog prototypes (`buttap` etc.), standalone `bilinear`/`freqs`, `cheb2ord`/`ellipord`, form conversions (§2.1 follow-on); `fir2`/`firls`/`firpm`/`firrcos`/`kaiserord` (§2.2); `phasez`/`zerophase`/Gustafsson IC for `filtfilt` (§2.5); spectral / time-frequency / multirate (§3, §4); `digitalFilter` system object (§5.1). Apps, GUIs, deep-learning, MATLAB Coder, Python coexecution explicitly carved out
