...
- [`signal_toolbox_roadmap.md`](signal_toolbox_roadmap.md): tiered plan for Signal Processing Toolbox compatibility. **Tier-1 + Tier-2 bulk are closed**. Tier-1: windows (17), polynomial helpers (`roots`/`poly`/`polyder`/`polyint`/`residue`), IIR (`butter`/`cheby1`/`cheby2` + `buttord`/`cheb1ord`), FIR (`fir1`/`sgolay`/`sgolayfilt`), filter response (`freqz`/`impz`/`stepz`/`grpdelay`), filter application (`filter`/`filtfilt`/`sosfilt`). Tier-2: transforms (`dct`/`idct`/`fwht`/`hilbert`/`goertzel`), spectral (`periodogram`/`pwelch`/`cpsd`/`mscohere`/`tfestimate`), linear prediction + parametric PSD (`levinson`/`lpc`/`aryule`/`arburg`/`pyulear`/`pburg`), time-frequency (`spectrogram`). **Open follow-ons**: `ellip`/`besself`/band variants/analog prototypes (§2.1); `fir2`/`firls`/`firpm` (§2.2); Gustafsson IC for `filtfilt` (§2.5); `dpss`/`pmtm`/`czt`/`stft`/`istft`/subspace methods (Tier-2 follow-on); multirate (§4.1), pulse measurements (§4.3), `digitalFilter` system object (§5.1). Apps, GUIs, deep-learning, MATLAB Coder, Python coexecution explicitly carved out
