...
- [`control_toolbox_roadmap.md`](control_toolbox_roadmap.md): tiered plan for Control System Toolbox compatibility. **Tier-0 baseline only** — no CST-specific surface shipped yet. Tier-1 is a numeric-prerequisite stack (non-symmetric `eig`, `schur` / `hess` / `qz`, `expm` / `logm`, `lyap` / `dlyap` / `lyapchol`, `care` / `dare`); ~5 weeks; gates everything. Tier-2 (~3 wk): model object constructors (`tf` / `ss` / `zpk` / `frd` / `pid` as classdefs with operator overloads), conversions (`c2d` / `d2c` / `canon` / `ss2ss`), time-domain (`step` / `impulse` / `lsim` / `initial` / `stepinfo`), frequency-domain (`bode` / `nyquist` / `sigma` / `freqresp` / `margin` / `dcgain`), interconnections (`series` / `parallel` / `feedback`), and `pidtune`. Tier-3 (~3 wk): state-space design (`lqr` / `dlqr` / `lqi` / `lqg` / `kalman` / `lqgreg`, `place` / `acker` / `estim`, `ctrb` / `obsv` / `gram`, `norm` H₂/H∞). Tier-4 (~3 wk): model reduction (`balred` / `balreal` / `modred` / `hsvd` / `minreal`), MIMO connection (`connect` / `sumblk` / `append` / `lft`), Padé / time delays. Tier-5: passivity / sectors (`isPassive` / `getPassiveIndex` / `getSectorIndex`, `loopsens`). All apps (Control System Designer, Linear System Analyzer, Control System Tuner, Model Reducer, PID Tuner, Linearizer), Simulink linearization, LPV/LTV simulation, sparse second-order models, `systune` / `looptune` / `hinfstruct` / `TuningGoal.*` / `genss`, and Robust Control / SysID / MPC / Adaptive Control toolbox bridges are explicitly carved out
