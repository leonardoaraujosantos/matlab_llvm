...
- [`signal_toolbox_roadmap.md`](signal_toolbox_roadmap.md): tiered plan for Signal Processing Toolbox compatibility. **Shipped**: windows tail (17 windows), polynomial helpers (`roots`/`poly`/`polyder`/`polyint`/`residue` distinct-pole), IIR lowpass design (`butter`/`cheby1`), `freqz`. **Open**: `cheby2`/`ellip`/`besself` + analog prototypes + band variants + order helpers (§2.1 follow-on); FIR design (§2.2); `filtfilt`/`sosfilt`/`impz`/`grpdelay` (§2.5); spectral / time-frequency / multirate (§3, §4); `digitalFilter` system object (§5.1). Apps, GUIs, deep-learning, MATLAB Coder, Python coexecution explicitly carved out
