...
`grading_timeout_s`. Health check (`/healthz`) gains a DB-connectivity probe.
...
user_files       id(uuid PK), user_id→users(cascade), rel_path, size_bytes,
                 content_type, created_at, updated_at, unique(user_id, rel_path)
                 # bytes live on the per-user workspace volume; this is the catalog
...
- **T0 — DB foundation.** SQLAlchemy async engine/session + `Base`; asyncpg;
  ...
  `require_auth` → upsert `users`, bump `last_seen_at`); `user_files` catalog
  wired into the existing `/v1/files`; `/healthz` DB probe.
...
- **Deploy (Coolify):** add a Postgres resource → `DATABASE_URL`; run
  migrations on release; enable backups.
