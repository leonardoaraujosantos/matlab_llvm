...
- **Phase 7.4 — `pdepe` extensions.** Multi-component systems (`npde > 1`); axis-of-symmetry handling for `xmesh(1) = 0` with `m > 0`; plumbing `odeset` through to the time integrator. Then 2-D parabolic via `pdepe`-style on a tensor-product mesh.
