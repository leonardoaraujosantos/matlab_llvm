...
- **Phase 7.3 — Higher-order stiff solver.** `ode15s` (variable-order BDF + Newton iteration on top of the shipped FD-Jacobian / LU infrastructure from 7.1). Other stiff variants (`ode23t`, `ode23tb`, `ode15i` for DAEs) drop in cheaply once BDF lands. Unlocks efficient numerical PDE via `pdepe` (method-of-lines) on stiff parabolic problems — `ode23s` already makes simpler MOL discretizations tractable.
