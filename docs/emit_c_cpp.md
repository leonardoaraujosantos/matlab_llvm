...
- **Mode-matched banner.** `// Generated by matlabc -emit-cpp. Do not edit.`
  in C++ mode, `-emit-c` in C mode — used to always say `-emit-c`.
- **Empty prolog sections suppressed.** When every runtime call got
  substituted, the `extern "C" {}` block and its heading comment are
  elided entirely — no dangling `// Runtime prototypes ...` header above
  an empty block. Same for the string-constant block and the
  forward-declaration block when there's nothing to declare.
- **Dead `llvm.mlir.global`s pruned.** String literals whose only uses
  fed `matlab_disp_str` callsites that got substituted are dropped from
  the emitted prolog — no `static const char __matlab_str0[] = "..."`
  stubs hanging around after their consumers went inline.
- **Integer-valued doubles formatted as `N.0`.** An arith/LLVM constant
  holding `42.0` prints as `42.0`, not `42`. Keeps the type unambiguous
  in varargs `printf("%g\n", ...)` context and reads more like the
  MATLAB value the user wrote. Removes the `(double)` cast that the
  disp-substitution used to need.
- **`#line` directives are forward-only.** A synthesized op that
  inherits an earlier line's location (scf.if closing brace, main's
  return reusing the first-stmt loc) no longer re-points the debugger
  backward; only forward progression (and cross-function / cross-file
  switches) emits a directive.
- **Single-use function calls inline into their user.** A `func.call`
  or `llvm.call` with exactly one use in the same block and no
  intervening store / call between producer and user is marked
  inlineable — its definition is suppressed and the call text is
  substituted at the use site. `double v0 = fact(1); disp(v0);`
  collapses to `std::cout << fact(1) << '\n';`. Indirect calls
  (function-pointer dispatch) and void-return calls stay put.
