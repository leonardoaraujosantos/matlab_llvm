...
| `signal_lqr`          | ✓ | `K` (gain matrix), `sign: -1`                                            | Control (#343) — static state-feedback gain `u = -K·x` (LQR / pole placement). `K` is an M×N matrix literal, `x` the vector input; output is the M-vector `u` (scalar for a 1×N row). `sign: 1` emits `+K·x` |
| `signal_dnn_predict`  | ✓ | `W1`, `b1`, `W2`, `b2` (matrix literals), `activation: "relu"`           | Deep Learning (#343) — one-hidden-layer MLP inference in the loop: `y = W2·act(W1·x + b1) + b2`. `activation`: `relu` (default) / `tanh` / `sigmoid` / `linear`. Input dim = cols of `W1`, output dim = rows of `W2` |
| `signal_rf_2port`     | ✓ | `S` (2×2 matrix literal)                                                | RF (#343) — memoryless scattering 2-port: reflected waves `b = S·a` from the incident-wave vector `a = [a1, a2]`, `S = [S11 S12; S21 S22]`. Cascade instances for an ideal time-domain network |
| `signal_pose_transform` | ✓ | `x: 0`, `y: 0`, `theta: 0` (rad)                                       | Nav/Robotics (#343) — 2-D rigid-body transform of a point: `out = R(θ)·[px, py] + [x, y]`. The point is the vector input; the pose is from params. For body↔world frame conversion in a fusion/SLAM loop |
| `signal_rl_agent`     | ✓ | `W1`/`b1`/`W2`/`b2`, `actionType: "discrete"`, `actionScale: 1.0`       | Reinforcement Learning (#343) — trained deterministic policy in the loop. The MLP maps state → raw output; `discrete` emits the `argmax` action index (e.g. DQN), `continuous` emits `actionScale·tanh(raw)` per output (bounded, e.g. DDPG) |
...
| `signal_spectrum`     | ✓ | `n` (frame size)                                                        | DSP (#343) — power-spectrum sink. Real `n`-point frame → `|X[k]|² = Re² + Im²` (width `n`). Pairs with a `signal_window` taper for spectrum analysis |
| `signal_dwt`          | ✓ | `n` (frame size)                                                        | Wavelet (#343) — 1-level Haar DWT. Frame → `[approx; detail]` (width `n`), `approx[k]=(x[2k]+x[2k+1])/√2`, `detail[k]=(x[2k]−x[2k+1])/√2` |
| `signal_idwt`         | ✓ | `n` (frame size)                                                        | Wavelet (#343) — inverse 1-level Haar. `[approx; detail]` frame → reconstructed signal. `dwt → idwt` is the identity |
| `signal_lowpass`      | ✓ | `alpha: 0.1`, `sampleTime: 1.0`                                         | DSP (#343) — one-pole lowpass (EMA) `H(z)=α/(1-(1-α)z⁻¹)`, unity DC gain. `discrete_filter` preset |
...
| `signal_udp_send` | ✓ | `host: "127.0.0.1"`, `port: 5000` | network-io — transmit the input wire to a UDP endpoint, **once per major step** (never inside an RK4 minor stage, so the continuous integration stays deterministic). A scalar input sends one `float64`; a wide input (e.g. a `signal_actor3d` 9-wide transform, or a 3-mux position) streams every element. The input is passed through to the output port for chaining/logging. |
| `signal_udp_recv` | ✓ | `port: 5000`, `host: "127.0.0.1"`, `initialValue: 0.0` | network-io — bind a UDP socket and output the most recent `float64` received, draining once per major step. **Loop-breaker** (its output does not depend on this step's input) so it can sit in a feedback path; holds its last value (`initialValue` before the first packet) on starvation, so a slow/absent peer never stalls the solver. |
| `signal_tcp_send` | ✓ | `host: "127.0.0.1"`, `port: 5000` | network-io — like `signal_udp_send` over a TCP stream (connects to the server on `reset()`). |
| `signal_tcp_recv` | ✓ | `host: "127.0.0.1"`, `port: 5000`, `initialValue: 0.0` | network-io — like `signal_udp_recv` over TCP (binds + listens, accepting one client lazily on first data). Loop-breaker; holds last value. |
