...
calls into. Twenty-six toolbox surfaces ship today:
...
| **Financial** *(all 7 tiers)* | [`runtime/toolbox/finance/runtime_finance.cpp`](runtime/toolbox/finance/runtime_finance.cpp) + [`runtime/toolbox/finance/finance_classdefs.m`](runtime/toolbox/finance/finance_classdefs.m) | **All 7 tiers shipped** — hand-coded over the shipped Optim / LAPACK / Stats / Cairo base (no QuantLib). **T1** dates (`yearfrac` bases 0/1/2/3/6/12, `busdate`/`eomdate`/`daysdif`/…), cash flows (`pvfix`/`fvfix`/`pvvar`/`fvvar`/`irr`/`payper`/`amortize`), depreciation, bond pricing (`bndprice`/`bndyield`/`bnddurp`/`bndconvp` + T-bills), returns + indicators (`tick2ret`/`ret2tick`/`sma`/`bolling`/`rsindex`). **T2** performance metrics (`sharpe`/`sortino`/`inforatio`/`maxdrawdown`/`lpm`/`portalpha`) + Black-Scholes (`blsprice` + 6 Greeks + `blsimpv`). **T3** `Portfolio` mean-variance classdef — `setAssetMoments`/`setBounds`/`setDefaultConstraints`, `estimateFrontier`/`estimateFrontierByReturn`/`estimateFrontierByRisk`/`estimateMaxSharpeRatio`/`estimateBounds`/`estimatePortMoments`/`plotFrontier` (closed-form Lagrangian + box/budget projection). **T4** credit + missing-data regression — `ecmnmle`/`ecmncov` (ECM), `mvnrmle`/`capm`, `transprob`, `cdsbootstrap`/`cdsspread`/`cdsprice`, `creditscorecard` (logistic IRLS) + `fitmodel`/`probdefault`/`score`. **T5** `PortfolioCVaR` + `PortfolioMAD` (scenario risk via projected subgradient, shared methods dispatch on a `RiskKind` discriminant) + `backtest`/`backtestSummary`. **T6** SDE Monte Carlo — `bm`/`gbm`/`cir`/`hwv` + `simByEuler`/`simBySolution`, `haltonseq` QMC, `optpricemc`. **T7** `blacklitterman` posterior, `riskparity`/`riskbudget`/`riskcontribution`. Headlines `efficient_frontier` (Markowitz) · `monte_carlo_blsprice` (GBM MC vs closed-form, <1%) · `black_litterman` · `credit_scorecard` · `backtest_strategies` · `using_timetables_in_finance` (full timetable workflow). 20 gating tests. Carve-outs (GUI/RL/DL examples, Heston/Sobol, handle-backtest, LP-backed CVaR, WoE autobinning, robust SOCP, yield-curve bootstrap, option lattices) tracked in [`docs/financial_toolbox_status.md`](docs/financial_toolbox_status.md). See [`docs/financial_toolbox_roadmap.md`](docs/financial_toolbox_roadmap.md). |
| **Econometrics** *(all 6 tiers)* | [`runtime/toolbox/econ/runtime_econ.cpp`](runtime/toolbox/econ/runtime_econ.cpp) + [`runtime/toolbox/econ/econ_classdefs.m`](runtime/toolbox/econ/econ_classdefs.m) | **All 6 tiers shipped** — hand-coded over the shipped Optim / Stats / LAPACK / System-Identification base (no statsmodels/R). **T1** data prep (`price2ret`/`ret2price`/`hpfilter`), `autocorr`/`parcorr`/`crosscorr`, diagnostic tests (`lbqtest`/`archtest`/`aicbic`/`lratiotest`/`waldtest`/`lmtest`/`hac`/`fgls`), unit-root + stationarity (`adftest`/`pptest`/`kpsstest`/`lmctest`/`vratiotest`). **T2** `arima(p,D,q)` conditional-mean classdef — `estimate` (Hannan-Rissanen) / `infer` / `forecast` (recursive MMSE) / `simulate`. **T3** `garch`/`egarch`/`gjr` conditional-variance classdefs — Gaussian-MLE `estimate` (Nelder-Mead over the variance recursion) / `infer` / `forecast` / `simulate`. **T4** `varm` (VAR) — equation-wise-OLS `estimate` / `forecast` / `simulate` / `irf` (orthogonalized) + cointegration tests `egcitest` (Engle-Granger) / `jcitest`/`jcontest` (Johansen trace). **T5** `ssm`/`dssm` state-space — `estimate` (Kalman-filter ML) / `filter` (Kalman) / `smooth` (RTS) / `forecast`. **T6** `bayeslm` (diffuse-prior Bayesian linear regression) + `dtmc` Markov chains (`asymptotics` stationary dist / `simulate`). Headlines `stationarity_workflow` · `arima_cpi_forecast` (Box-Jenkins) · `garch_volatility` · `var_macro` (VAR + IRF) · `ssm_kalman` (smoothing cuts SSE 12×) · `bayeslm_regression`. 7 gating tests. Carve-outs (Econometric Modeler GUI, `regARIMA`, Markov-switching `msVAR`, DSGE, HMC samplers, Bayesian/particle state-space) tracked in [`docs/econometrics_toolbox_roadmap.md`](docs/econometrics_toolbox_roadmap.md). |
...
| **Robotics System** *(Tiers 1–6)* | [`runtime/toolbox/robotics/runtime_robotics.cpp`](runtime/toolbox/robotics/runtime_robotics.cpp) + [`runtime/toolbox/robotics/robotics_classdefs.m`](runtime/toolbox/robotics/robotics_classdefs.m) | **All 6 tiers shipped** — manipulator FK/IK + mobile-robot planning + collisions + RRT, hand-coded over the shipped Sensor Fusion quaternion + Optim `lsqnonlin`/`fminunc` + the PDE STL importer (URDF mesh import deferred to a follow-on). **T1** `se3`/`so3` value types + the full `trvec2tform`/`tform2trvec`/`rotm2tform`/`tform2rotm`/`eul2tform`/`tform2eul`/`axang2rotm`/`rotm2axang`/`axang2tform`/`tform2axang`/`quat2tform`/`tform2quat` conversion surface + `homtrans` + `wrapToPi`/`wrapTo2Pi` + `vecnorm`. **T2** `rigidBodyTree` over packed DH/JointTypes/JointLimits matrices + `addBody` + DH-parameter `setFixedTransform` (via `addBody`) + `loadrobot('planar2'/'planar3')` baked-in models + `getTransform` (FK via DH chain) + `geometricJacobian` (6×N spatial Jacobian via z×(p_e − p_i) per joint) + `homeConfiguration` + `randomConfiguration`. **T3** (**headline `ik_path_trace.m`**) `inverseKinematics` — Levenberg-Marquardt damped least-squares with adaptive λ over the 6-vector pose error (position + axis-angle orientation), multi-return `[q, iters, exitflag, err]`; `constraintPoseTarget` carrier. Also **`importrobot`** parses a URDF (hand-rolled XML reader → fixed-transform tree with `<inertial>` mass/COM/inertia + `<origin>`/`<axis>`/`<limit>`), and `generalizedInverseKinematics` adds the multi-constraint LM solver (`constraintPositionTarget`/`constraintOrientationTarget`). **T4** `cubicpolytraj` (Hermite cubic), `trapveltraj` (3-phase trapezoidal), `transformtraj` (slerp+lerp), and **full rigid-body dynamics** — `inverseDynamics` (recursive Newton-Euler), `massMatrix` (composite-rigid-body via unit-acceleration RNEA), `forwardDynamics` (M⁻¹(τ−bias), inverts ID to machine precision), `gravityTorque`/`velocityProduct`/`centerOfMass`. **T5** (**headline `diffdrive_prm.m`**) `differentialDriveKinematics`/`unicycleKinematics`/`bicycleKinematics`/`ackermannKinematics` + `derivative` (the full "Mobile Robot Kinematics Equations" model set) + `binaryOccupancyMap` (grid + `setOccupancy`/`getOccupancy`/`checkOccupancy`) + `mobileRobotPRM` (uniform free-cell sampling + connection-within-distance + Dijkstra `findpath`) + `controllerPurePursuit`. **T6** `collisionBox`/`collisionSphere`/`collisionCylinder`/`collisionCapsule` primitives + **orientation-aware GJK** `checkCollision` (support-function Gilbert-Johnson-Keerthi over the Minkowski difference, exact separation distance) + simplified `manipulatorRRT` over configuration space with FK-based obstacle check + step-bounded extension. Headlines `ik_path_trace.m` (T1-T3 — 4 waypoints traced with err≈0) · `diffdrive_prm.m` (T5) · `arm_dynamics.m` (T4 — FD/ID round-trip 3.9e-16) · `ik_2d_circle.m` (2-D IK circle trace, the MathWorks 2-D-IK workflow) · `mobile_kinematics.m` (all four mobile models). 8 gating tests. Carve-downs (`analyticalInverseKinematics`, `jointSpaceMotionModel`/`taskSpaceMotionModel`, `bsplinepolytraj`/`minjerkpolytraj`/`contopptraj`, `controllerVFH`, `rangeSensor`, `stateSpaceSE3`, `manipulatorCHOMP`, EPA penetration depth + `collisionMesh`, `stateEstimatorPF`, `rateControl`, per-body `rigidBody`/`rigidBodyJoint` object API + callable `ik`/`controller`) tracked in [`docs/robotics_toolbox_roadmap.md`](docs/robotics_toolbox_roadmap.md). |
| **Sensor Fusion and Tracking** *(Tiers 1–6)* | [`runtime/toolbox/fusion/runtime_fusion.cpp`](runtime/toolbox/fusion/runtime_fusion.cpp) + [`runtime/toolbox/fusion/fusion_classdefs.m`](runtime/toolbox/fusion/fusion_classdefs.m) | **All 6 tiers shipped — inertial-nav + multi-object tracking + track fusion + metrics** — hand-coded over the shipped EKF/UKF cores from System Identification Tier-5 (no Eigen/GTSAM/Ceres). **T1** `quaternion` value type (N×4 [w x y z] storage) + Hamilton product / `conj`/`norm`/`normalize`/`inverse` / `slerp` / `dist` / `rotatepoint`/`rotateframe` / `quat2eul`/`eul2quat`/`quat2rotm`/`rotm2quat` + `ecompass` (Davenport's q-method) + new generic-math builtins `cross`/`dot`/`deg2rad`/`rad2deg`/`mvnrnd` (T1.7 core gaps). **T2** `trackingKF` (linear KF over `constvel`/`constacc`/`constturn` motion models + `cvmeas`/`cameas`/`ctmeas` measurement models) + `trackingEKF`/`trackingUKF` (vector-y re-skin of the shipped EKF/UKF cores via the function-handle ABI) + `objectDetection` container + `initcvekf`/`initctekf` initialisers. **T3** `imuSensor` + `gpsSensor` + `ahrsfilter`/`imufilter`/`complementaryFilter` (Mahony-style complementary orientation) + **`insfilterMARG` (T3 headline)** (16-state quaternion + position + velocity + bias EKF fusing IMU + GPS) + `allanvar` noise analysis. **T4** `waypointTrajectory` (position interpolation via `lookupPose(t)`) + WGS-84 coordinate-frame helpers `lla2ned`/`ned2lla`. **T5** `assignmunkres` (O(n³) Hungarian assignment) + `objectTrack` container + **`trackerGNN` (T5 headline)** (GNN tracker over constvel trackingEKFs with Mahalanobis gating, Munkres assignment, and confirmation/age logic). **T6** **`trackFuser` (covariance intersection)** with trace-minimizing line search over ω + `trackGOSPAMetric` / `trackOSPAMetric` (Munkres-based set-distance metrics) + `trackErrorMetrics` RMSE + `rtsSmoother` Rauch-Tung-Striebel backward pass — closes the T6 headline `track_fusion_metrics.m`. Headlines `imu_gps_fusion.m` (T3) · `gnn_air_traffic.m` (T5) · `track_fusion_metrics.m` (T6) · `quaternion_rotations.m`. 5 gating tests. Carve-downs (full Joseph-form insfilterMARG, `tune`/`tunerconfig`, `trackingIMM`/`GSF`/`PF`, kinematicTrajectory/geoTrajectory/trackingScenario/theaterPlot, `assignjv`/`auction`/`sd`, `trackerJPDA`/`TOMHT`/`PHD`, `fusionRadarSensor`, OOSM retrodiction) tracked in [`docs/sensor_fusion_toolbox_roadmap.md`](docs/sensor_fusion_toolbox_roadmap.md). |
| **Navigation** *(Tiers 1–6)* | [`runtime/toolbox/navigation/runtime_navigation.cpp`](runtime/toolbox/navigation/runtime_navigation.cpp) + [`runtime/toolbox/navigation/navigation_classdefs.m`](runtime/toolbox/navigation/navigation_classdefs.m) | **All 6 tiers shipped — the autonomy capstone, hand-coded over the just-shipped Robotics + Sensor Fusion bases** (no g2o/GTSAM/Ceres/PCL/OMPL). **T1** `occupancyMap` (probabilistic grid + `setOccupancy`/`getOccupancy`/`checkOccupancy`/`inflate` + world limits) + `stateSpaceSE2`/`stateSpaceDubins` (`distance`/`interpolate`/`sampleUniform` + state bounds) + `validatorOccupancyMap` (`isStateValid`/`isMotionValid` via interpolated sweep) + `navPath`. **T2** (**headline `nav_rrt_plan.m`**) `plannerRRT` + `plannerRRTStar` (sample → nearest → steer → collision-check → extend, goal-biased; RRT\* min-cost-parent + rewiring) + `plannerAStarGrid` (8-connected grid A\*) + `plan` → packed `[meta; states]` + `shortenpath` greedy shortcut smoothing. **T3** `lidarScan` (Ranges/Angles ↔ Cartesian) + `matchScans` (point-to-point ICP via 2-D cross-covariance closed form) + `lidarSLAM` (`addScan` incremental scan-to-scan trajectory accumulation). **T4** `poseGraph` (SE2 nodes + relative-pose edges, `addRelativePose` chaining + loop-closure edges) + `optimizePoseGraph` (SE2-manifold Gauss-Newton over relative-pose residuals, node 1 fixed, recovers an exact unit square). **T5** `controllerVFH` (Vector Field Histogram reactive steering — polar obstacle histogram, obstacles enlarged by robot radius + safety) + `monteCarloLocalization` (particle filter on the occupancyMap — odometry motion model + likelihood-field measurement update + low-variance resampling) + `stateEstimatorPF` (generic linear-Gaussian PF — `initialize`/`predict`/`correct`/`getStateEstimate`). **T6** `gnssSensor` + `gnssconstellation` + `pseudoranges` + `receiverposition` (iterative least-squares pseudorange trilateration over WGS-84 ECEF, solving 3-D position + clock bias) + `referencePathFrenet` (`global2frenet`/`frenet2global`) + `trajectoryGeneratorFrenet` (`connect` — quintic Frenet lane-change). Examples mirror the MathWorks UG pages: `nav_rotations.m` · `nav_imu_intro.m` · `nav_ground_vehicle.m` (IMU+GPS → `insfilterMARG`) · `nav_rrt_plan.m` (Plan Paths Using RRT) · `nav_vfh_avoid.m` · `nav_mcl_localize.m` · `nav_gnss_position.m` · `nav_frenet_planner.m`. 11 gating tests. Carve-downs (`stateSpaceSE3`/Reeds-Shepp, `plannerHybridAStar`/`plannerBiRRT`, `factorGraph`/`poseGraph3D`, `odometryMotionModel`/`likelihoodFieldSensorModel` standalone objects + PF handle forms, `nmeaParser`/`rinexread`/`insEKF`, real almanac/ephemeris constellation, MPNet/visual-SLAM/3-D point-cloud SLAM, ROS/Gazebo/Simulink) tracked in [`docs/navigation_toolbox_roadmap.md`](docs/navigation_toolbox_roadmap.md). |
| **Deep Learning** *(all 6 tiers + DL HDL H1–H5)* | [`runtime/toolbox/dlnet/runtime_dlnet.cpp`](runtime/toolbox/dlnet/runtime_dlnet.cpp) + [`runtime/toolbox/dlnet/runtime_onnx.cpp`](runtime/toolbox/dlnet/runtime_onnx.cpp) + [`runtime/toolbox/dlnet/dlnet_classdefs.m`](runtime/toolbox/dlnet/dlnet_classdefs.m) | **All 6 tiers + DL HDL H1–H5 shipped** — hand-coded over the shipped matrix kernel (no PyTorch / TF / cuDNN / ONNX-runtime). `dlarray` carries data + a tape-node id; the forward pass uses **natural operator overloading** (`W*X + b`, `+`, `.*`, `./`, reductions, all with numpy-style broadcasting) + activation/reduction/loss methods and **records onto a thread-local reverse-mode tape** so **`dlgradient(loss, var)`** sweeps it (T2 keystone; verified vs central finite differences to **1.24e-10**). **T1 inference**: `dlnetwork` carrier (`addFC` / `addRelu` / `addSigmoid` / `addTanh` / `addSoftmax` + `predict`), full activation surface (relu / sigmoid / tanh / softmax / gelu / swish / leakyrelu / elu / softplus) + losses (crossentropy / mse / l1loss / huber / l2loss), `imageDatastore` / `countEachLabel` / `splitEachLabel`. **T2 autodiff**: LayerNorm / GroupNorm / InstanceNorm / RMSNorm / BatchNorm (EMA-tracked at train, identity-with-stored-stats at infer); single-head Transformer encoder block; multi-head attention training (concatenated heads); dropout; VAE w/ reparameterization. **T3 training**: `trainnet` / `trainingOptions` driver (Adam over `Learnables`) **and** the custom training loop (`dlfeval` + `dlgradient` + `adamupdate` / `sgdmupdate` / `rmspropupdate` functional solvers); `augmentedImageDatastore` (random rotation/scale/translation + `gray2rgb` + per-batch resize); headless training monitor; single-device GPU training dispatch (`matlab_gpu_gemm` routing on forward/backward/solver). **T4 sequence / recurrent / attention**: `lstm` / `gru` / `bilstm` / `lstmp` (one custom tape node each, BPTT inside), `embed` (gather-forward + scatter-add-backward), functional scaled-dot-product attention via shipped `matmul` + `softmax` + `transpose`, **1-D conv sequence path** (`conv1d`). **T5 architectures + transfer learning**: residual blocks via overloaded `plus`; transfer learning (frozen encoder as plain matrices + autodiff head → 96% on a 3-class task); autoencoder; LSGAN-style GAN; Siamese twin embedding; **Neural ODE** (stepper-matrix form on the dense lane). **T6 tuning / viz / metrics / quantization**: Grad-CAM-style attribution on MLPs **and** image-domain `gradCAM` / `occlusionSensitivity` / `imageLIME` on the 4-D `SSCB` tensor; `tsne`; classification metrics (`accuracy` / `precision` / `recall` / `fScore` / `rocmetrics` / `aucroc`); `dlqcalibrate` + `dlqclip` activation-quantization; Bayesian HP search wrapping shipped `bayesopt`; dual-norm l∞ robustness check; **magnitude-based pruning** (zero bottom-k% by `|w|`, re-fine-tune over the custom loop); **programmatic experiment sweep harness** (`runExperiment(@trialFn, gridSpec, opts)`, Cartesian or Bayesian, per-trial result table — no GUI). **DL HDL H1–H5**: H1 `dlquantize` / `dlqscale` symmetric per-tensor INT8 weight quant (both double and INT8 at 100% accuracy, max logit drift ≈ 0.1); H2 fi-typed SV emission of a quantized MLP (`dlhdl_quant_mlp.m` — Q16.8 2-2-1, ~15 lines synthesizable, Verilator + Yosys clean); H3 cocotb bit-accuracy via the **`% hdl: precise_fi`** opt-in pragma (Sema-mono on the HW lane so fi-grown widths thread through `matlab.matmul` / `matlab.add` result types, EmitSV emits 64-bit intermediates + FL-rescaled bias — 41/41 vectors agree with the Python reference; closes [issue #75](https://github.com/leonardoaraujosantos/matlab_llvm/issues/75)); H4 LSTM-on-FPGA (`dlhdl_rnn_cell` / `dlhdl_lstm_cell` / `dlhdl_lstm_step` — recurrent fi kernel, `persistent` h_state/c_state lowers to `always_comb` + `always_ff` shift-register pattern; cocotb 44/44); **H5 minimal ONNX inference-graph importer + writer** — hand-rolled Protobuf reader/writer in `runtime_onnx.cpp` (~56 op handlers: Conv2D / Gemm / MatMul / BatchNorm / ReLU / MaxPool / AvgPool / Add / Concat / Softmax / Sigmoid / Tanh / Reshape / Transpose + 40 more); `dl_onnx_roundtrip.m` and `dl_onnx_ops_coverage.m`. **emit-c parity (G)**: handle-classdef `void*` typedef + value-receiver method bodies + LiveGlobals scan in `EmitC` so every dlarray-using test lifts cleanly through emit-c bit-exact to the LLVM lane; classdef-array literal `[obj1; obj2; obj3]` detects all-classdef-instance leaves and routes to a generic object-array carrier (`matlab_dlnet_oa_new` / `oa_append`); two semantic-divergence holdouts (`dl_mha_train` minor FP rounding diff vs LLVM; `dl_cnn_classifier` flatlines under emit-c at uniform softmax) skipped on emit-c only. **39 `dl_*.m` gating tests + 44 examples** under `examples/dlnet/`. **Carve-outs** (per the roadmap §10): Deep Network Designer + Experiment Manager apps (GUIs; programmatic engines ship), all Simulink Deep-Learning blocks (the `mflowLink` lane is the answer), full external-framework import/export (`importNetworkFromPyTorch` / `importNetworkFromTensorFlow` / `exportNetworkToTensorFlow`; the minimal inference-graph ONNX importer ships as H5), real named pretrained weight blobs (AlexNet/ResNet/YOLO/BERT — 100s of MB), multi-GPU / cluster / cloud training, disk-backed big-data datastores, RL / CV / Audio / Text Analytics toolbox deps, **DL HDL silicon** (bitstream / board / LIBIIO / vendor synthesis — simulation surface only), GPU/CPU `'lib'` quantizer targets, legacy "Neural Network Toolbox" shallow-net surface. |
| **Reinforcement Learning** *(Tiers 1–4; T5 implemented, blocked)* | [`runtime/toolbox/rl/runtime_rl.cpp`](runtime/toolbox/rl/runtime_rl.cpp) + [`runtime/toolbox/rl/rl_classdefs.m`](runtime/toolbox/rl/rl_classdefs.m) | **Tiers 1–4 shipped — the deep agents reuse the shipped Deep Learning autodiff tape, no new numerical kernel.** The RL runtime builds each actor/critic forward pass as `dlarray` shells (`matlab_obj_new`) and drives the existing `matlab_dlnet_*` C-ABI (`mtimes`/`plus`/`relu`/`softmax`/`log`/`sum`/`mse` + `matlab_dlnet_grad`) directly from C++ — only the Adam moment-update + replay/episode orchestration live RL-side (zero dlnet changes, zero forward/backward duplication). **T1 (tabular, autodiff-free)**: observation/action specs (`rlNumericSpec`/`rlFiniteSetSpec` + `getObservationInfo`/`getActionInfo`), `rlPredefinedEnv("BasicGridWorld")` + `rlMDPEnv(nextState,reward)` builder, `rlTable`/`rlQValueFunction`, and `rlQAgent` (Q-learning) / `rlSARSAAgent` whose ε-greedy TD loop runs entirely in the runtime over the env's next-state/reward tensors; `train`/`sim`/`getCritic`/`getLearnableParameters`. **T2 (control env + policy use)**: `rlPredefinedEnv("CartPole-Discrete")` continuous-state env (Barto dynamics), greedy `sim` rollout, and the policy-use accessors `getAction`/`getMaxQValue` + `getGreedyPolicy` → `rlMaxQPolicy` (net + tabular dispatch). **T3 (deep value-based)**: `rlDQNAgent(obsInfo,actInfo)` auto-builds a 24-unit MLP critic + experience replay + target network + ε-greedy; the TD-error loss (`mse`) → `dlgradient` → Adam runs on the reused tape. **T4 (policy gradient)**: `rlPGAgent` softmax-policy actor, REINFORCE with discounted normalized returns, the `−Σ logπ·Ĝ` loss assembled on the tape (`softmax`→`log`→masked `sum`). Headlines `examples/rl/gridworld_qlearning.m` (Q-learning + SARSA both reach the optimal 11.0) · `mdp_qlearning.m` (8-state MDP, optimal 13.0) · `cartpole_dqn.m` (DQN balances the pole 269 steps vs random ≈10–20) · `cartpole_reinforce.m` (REINFORCE 391 steps) · `use_trained_policy.m`. 5 gating tests. **T5 (continuous control / DDPG)** is implemented and wired end-to-end (`rlPredefinedEnv("Pendulum-Continuous")`, `rlDDPGAgent` deterministic actor + Q(s,a) critic + target nets + OU noise, the deterministic-policy-gradient step where the actor gradient flows through the critic via tape `vertcat([s; actor(s)])`) but ships **no demo yet** — blocked on the autodiff tape's training-scale memory model ([#82](https://github.com/leonardoaraujosantos/matlab_llvm/issues/82)) + convergence tuning. **Carve-outs**: RL Designer app, Simulink/Simscape environments, `rlFunctionEnv` custom-function env (blocked on function-handle gaps [#80](https://github.com/leonardoaraujosantos/matlab_llvm/issues/80)/[#81](https://github.com/leonardoaraujosantos/matlab_llvm/issues/81)), parallel/GPU, multi-agent, recurrent actors, offline/evolutionary training, hardware deploy. See [`docs/reinforcement_learning_toolbox_roadmap.md`](docs/reinforcement_learning_toolbox_roadmap.md). |

Headless **plotting** (Cairo backend → PNG/SVG/PDF, plus `getframe` +
`VideoWriter` animation capture → MP4/AVI via libav) is on by default
(`MATLAB_LLVM_WITH_PLOT` / `MATLAB_LLVM_WITH_PLOT_FFMPEG`, opt out with
`=OFF`; [`docs/plotting.md`](docs/plotting.md));
...
- [`docs/dae_solver_roadmap.md`](docs/dae_solver_roadmap.md): **Verilog-A analog circuit simulator** — the *tractable-Simscape* path (acausal physical simulation with no Simscape DSL, no Simulink, no Pantelides). 10 tiers (~14 wk minimal cut T1–6): DAE solver core (mass-matrix `ode15s` + fully-implicit `ode15i` + `decic`, extending the shipped `ode23s` LU/FD-Jacobian) → MNA assembler → full analog Verilog-A language (`ddt`/`idt`/`idtmod`/`laplace_*`/`transition`/`cross`-events) → DC/transient/AC/noise/RF analyses → multi-discipline (electrothermal) + structural netlists. **Heavy runtime reuse**: MNA over sparse triplets + GMRES-ILU, `laplace_*` over `tf2ss`/`lsim`, AC over complex-matrix ops, harmonic-balance + Monte-Carlo over **GPU GEMM**. The inverse of the shipped Verilog-A *emit* lane (`verilog_a_plan.md`); behavioral/causal VA lowers to mflowLink instead. Headlines `diode_rectifier.m` / `amp_ac_noise.m`
