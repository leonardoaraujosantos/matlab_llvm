...
| **Deep Learning** *(Tiers 1–5 + HDL H1)* | [`runtime/toolbox/dlnet/runtime_dlnet.cpp`](runtime/toolbox/dlnet/runtime_dlnet.cpp) + [`runtime/toolbox/dlnet/dlnet_classdefs.m`](runtime/toolbox/dlnet/dlnet_classdefs.m) | **Tiers 1–4 shipped over the 2-D dense surface** — hand-coded over the shipped matrix kernel (no PyTorch/TF/cuDNN/ONNX-runtime). `dlarray` carries data + a tape-node id; the forward pass uses **natural operator overloading** (`W*X + b`) + activation/reduction/loss methods and **records onto a thread-local reverse-mode tape** so **`dlgradient(loss, var)`** sweeps it (T2; verified vs central finite differences to **1.24e-10**). **T3** custom training loop (`dl_mlp_train.m`: MLP → 100%, loss 1.80→0.01). **T4 recurrent**: functional **`lstm`** (`dl_lstm_sequence.m` first-bit-memory → 100%, loss 6→0); **`gru`** (same task → 100%, loss 6→0); **`bilstm`** (packed [forward; backward] weights, both directions BPTT'd in one tape node); **`lstmp`** (projected hidden, `dlgradient` accumulates `dP` alongside `dW/dR/db`). **T4 attention**: scaled-dot-product `V * softmax(transpose(K) * Q)` composes from existing ops via the added `transpose` (`dl_attention.m`: associative-recall — softmax peaks on the matching key, loss 1→0). **T4 embedding**: `embed(E, idx)` — gather-forward + scatter-add-backward; repeated tokens correctly accumulate (`dl_embed_train.m`: learns a 3×5 table to per-element error < 0.01). Activation/op dispatch fires *only* when an arg is `dlarray`-pinned (recursing through operators + ctor calls), so the matrix `tanh`/`sum`/`log`/`exp`/`transpose` lanes are unaffected. **T5 architectures + transfer learning** ship as the *functional patterns* the object-array `dlnetwork` API would otherwise wrap: residual blocks (`y = relu(W2*relu(W1*x+b1)+b2) + x`) work since `plus` is already overloaded — `dl_residual_train.m` trains a 4-layer skip-connection MLP via the autodiff; transfer learning by keeping the pretrained encoder as **plain numeric matrices** (no dlarray → no gradient flow) and training only the head as a dlarray — `dl_transfer_learn.m` adapts a frozen 4→6 encoder to a 3-class downstream task (loss 1→0, ≥ 96% accuracy). **11 gating tests**, 9 examples. **Carve-downs**: convolution + 4-D `SSCB` tensors (no rank-N type yet — see [`any_shape_roadmap.md`](any_shape_roadmap.md) Tier C); object-array `dlnetwork`/layer container (no classdef array literals — the functional/custom-loop form is the shipped surface); `dlgradient` returns a matrix not a `dlarray`; built-in `trainnet`/`trainingOptions` + functional solvers `adamupdate`/`sgdmupdate`; the `freezeLayers`/`replaceLayer`/`connectLayers` object-array APIs (the *patterns* they encapsulate ship as `dl_transfer_learn.m`'s custom training loop). **DL HDL Tier H1** (INT8 weight quantization) shipped — `dlquantize(W)` returns the dequantized weight matrix (rounded onto the int8 grid `[-127, 127]` per-tensor scaled at `max(abs(W))/127`); `dlqscale(W)` returns the scale.  Symmetric per-tensor quantization; software-side, no autodiff (post-training step). `dl_quantize_check.m`: trains the T3 MLP, INT8-quantizes every weight, re-runs inference — **both double and INT8 reach 100% accuracy** with max logit drift ≈ 0.1.  **HDL H2/H3** (emit the quantized forward as fi-typed SystemVerilog + cocotb bit-accuracy) follow next. **T6** (tuning/Grad-CAM/`dlquantizer`-INT8-calibration-app) tracked in [`docs/deep_learning_toolbox_roadmap.md`](docs/deep_learning_toolbox_roadmap.md). |
