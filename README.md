...
[![Run-tests](https://img.shields.io/badge/run--tests-578_%E2%9C%93-2EA44F.svg)](test/Run)
...
| **Sensor Fusion and Tracking** *(Tiers 1–5)* | [`runtime/toolbox/fusion/runtime_fusion.cpp`](runtime/toolbox/fusion/runtime_fusion.cpp) + [`runtime/toolbox/fusion/fusion_classdefs.m`](runtime/toolbox/fusion/fusion_classdefs.m) | **Tiers 1–5 shipped — inertial-nav half + multi-object tracking** — hand-coded over the shipped EKF/UKF cores from System Identification Tier-5 (no Eigen/GTSAM/Ceres). **T1** `quaternion` value type (N×4 [w x y z] storage) + Hamilton product / `conj`/`norm`/`normalize`/`inverse` / `slerp` / `dist` / `rotatepoint`/`rotateframe` / `quat2eul`/`eul2quat`/`quat2rotm`/`rotm2quat` + `ecompass` (Davenport's q-method) + new generic-math builtins `cross`/`dot`/`deg2rad`/`rad2deg`/`mvnrnd` (T1.7 core gaps). **T2** `trackingKF` (linear KF over `constvel`/`constacc`/`constturn` motion models + `cvmeas`/`cameas`/`ctmeas` measurement models) + `trackingEKF`/`trackingUKF` (vector-y re-skin of the shipped EKF/UKF cores via the function-handle ABI) + `objectDetection` container + `initcvekf`/`initctekf` initialisers. **T3** `imuSensor` (accel + gyro + optional mag, with bias / noise-density / random-walk params) + `gpsSensor` (lla + velocity) + `ahrsfilter`/`imufilter`/`complementaryFilter` (Mahony-style complementary orientation) + **`insfilterMARG` (T3 headline)** (16-state quaternion + position + velocity + bias EKF fusing IMU + GPS) + `allanvar` noise analysis. **T4** `waypointTrajectory` (position interpolation via N×3 waypoints + time-of-arrival, `lookupPose(t)`) + WGS-84 coordinate-frame helpers `lla2ned`/`ned2lla`. **T5** `assignmunkres` (O(n³) Hungarian assignment) + `objectTrack` container + **`trackerGNN` (T5 headline)** (global-nearest-neighbour tracker over constvel trackingEKF filters with Mahalanobis gating, Munkres assignment, and confirmation/age logic). Headlines `imu_gps_fusion.m` (T3 IMU+GPS dead-reckoning) · `gnn_air_traffic.m` (T5 — 3-target tracking, all confirmed) · `quaternion_rotations.m`. 4 gating tests. Carve-downs (full insfilterMARG Joseph-form EKF, `tune`/`tunerconfig`, `trackingIMM`/`GSF`/`PF`, kinematicTrajectory / geoTrajectory / trackingScenario / theaterPlot, T6 trackFuser + GOSPA / OSPA / PHD / OOSM) tracked in [`docs/sensor_fusion_toolbox_roadmap.md`](docs/sensor_fusion_toolbox_roadmap.md). |
