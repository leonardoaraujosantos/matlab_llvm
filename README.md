...
| Signal Processing Toolbox (subset) | **Tier-1 lowpass design loop + Tier-2 spectral/parametric/time-frequency closed.** Tier-1: 17 windows; polynomial helpers (`roots`, `poly`, `polyder`, `polyint`, `residue`); IIR lowpass design (`butter`, `cheby1`, `cheby2`, `buttord`, `cheb1ord`); FIR (`fir1`, `sgolay`, `sgolayfilt`); filter impl (`filter`, `filtfilt`, `sosfilt`); response (`freqz`, `impz`, `stepz`, `grpdelay`). Tier-2: transforms (`dct`, `idct`, `fwht`, `hilbert`, `goertzel`); spectral analysis (`periodogram`, `pwelch`, `cpsd`, `mscohere`, `tfestimate`); linear prediction + parametric PSD (`levinson`, `lpc`, `aryule`, `arburg`, `pyulear`, `pburg`); time-frequency (`spectrogram`). See [`docs/signal_toolbox_roadmap.md`](docs/signal_toolbox_roadmap.md). |
...
- `184` execution tests in `test/Run/` plus `4` opt-in symbolic tests in `test/RunSym/`
