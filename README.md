...
[![Run-tests](https://img.shields.io/badge/run--tests-583_%E2%9C%93-2EA44F.svg)](test/Run)
...
| **Robotics System** *(Tiers 1–6)* | [`runtime/toolbox/robotics/runtime_robotics.cpp`](runtime/toolbox/robotics/runtime_robotics.cpp) + [`runtime/toolbox/robotics/robotics_classdefs.m`](runtime/toolbox/robotics/robotics_classdefs.m) | **All 6 tiers shipped** — manipulator FK/IK + mobile-robot planning + collisions + RRT, hand-coded over the shipped Sensor Fusion quaternion + Optim `lsqnonlin`/`fminunc` + the PDE STL importer (URDF mesh import deferred to a follow-on). **T1** `se3`/`so3` value types + the full `trvec2tform`/`tform2trvec`/`rotm2tform`/`tform2rotm`/`eul2tform`/`tform2eul`/`axang2rotm`/`rotm2axang`/`axang2tform`/`tform2axang`/`quat2tform`/`tform2quat` conversion surface + `homtrans` + `wrapToPi`/`wrapTo2Pi` + `vecnorm`. **T2** `rigidBodyTree` over packed DH/JointTypes/JointLimits matrices + `addBody` + DH-parameter `setFixedTransform` (via `addBody`) + `loadrobot('planar2'/'planar3')` baked-in models + `getTransform` (FK via DH chain) + `geometricJacobian` (6×N spatial Jacobian via z×(p_e − p_i) per joint) + `homeConfiguration` + `randomConfiguration`. **T3** (**headline `ik_path_trace.m`**) `inverseKinematics` — Levenberg-Marquardt damped least-squares with adaptive λ over the 6-vector pose error (position + axis-angle orientation), multi-return `[q, iters, exitflag, err]`; `constraintPoseTarget` carrier. **T4** `cubicpolytraj` (Hermite cubic with zero endpoint vel), `trapveltraj` (3-phase trapezoidal), `transformtraj` (slerp+lerp), compact `massMatrix` + `inverseDynamics` (CRBA-style + gravity-term RNEA) — `forwardDynamics`/`jointSpaceMotionModel`/`bsplinepolytraj`/`minjerkpolytraj`/`contopptraj` carved as follow-ons. **T5** (**headline `diffdrive_prm.m`**) `differentialDriveKinematics`+`derivative` + `binaryOccupancyMap` (grid + `setOccupancy`/`getOccupancy`/`checkOccupancy`) + `mobileRobotPRM` (uniform free-cell sampling + connection-within-distance + Dijkstra `findpath`) + `controllerPurePursuit`. **T6** `collisionBox`/`collisionSphere` primitives + `checkCollision` (sphere-sphere + AABB-AABB + sphere-box) + simplified `manipulatorRRT` over configuration space with FK-based sphere obstacle check + step-bounded extension. Headlines `ik_path_trace.m` (T1-T3 — 4 waypoints traced with err≈0) · `diffdrive_prm.m` (T5). 4 gating tests. Carve-downs (URDF `importrobot`, `analyticalInverseKinematics`, `generalizedInverseKinematics`, `forwardDynamics`/`jointSpaceMotionModel`, `taskSpaceMotionModel`, `bsplinepolytraj`/`minjerkpolytraj`/`contopptraj`, `unicycle`/`bicycle`/`ackermann`, `controllerVFH`, `rangeSensor`, `stateSpaceSE3`+`validatorOccupancyMap`, `manipulatorCHOMP`, full GJK+EPA collision + `collisionCylinder`/`Capsule`/`Mesh`, `stateEstimatorPF`, `rateControl`) tracked in [`docs/robotics_toolbox_roadmap.md`](docs/robotics_toolbox_roadmap.md). |
