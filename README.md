...
[![Run-tests](https://img.shields.io/badge/run--tests-593_%E2%9C%93-2EA44F.svg)](test/Run)
...
| **Navigation** *(Tiers 1–4)* | [`runtime/toolbox/navigation/runtime_navigation.cpp`](runtime/toolbox/navigation/runtime_navigation.cpp) + [`runtime/toolbox/navigation/navigation_classdefs.m`](runtime/toolbox/navigation/navigation_classdefs.m) | **Tiers 1–4 shipped — the autonomy capstone, hand-coded over the just-shipped Robotics + Sensor Fusion bases** (no g2o/GTSAM/Ceres/PCL/OMPL). **T1** `occupancyMap` (probabilistic grid + `setOccupancy`/`getOccupancy`/`checkOccupancy`/`inflate` + world limits) + `stateSpaceSE2`/`stateSpaceDubins` (`distance`/`interpolate`/`sampleUniform` + state bounds) + `validatorOccupancyMap` (`isStateValid`/`isMotionValid` via interpolated sweep) + `navPath`. **T2** (**headline `nav_rrt_plan.m`**) `plannerRRT` + `plannerRRTStar` (sample → nearest → steer → collision-check → extend, goal-biased; RRT\* min-cost-parent + rewiring) + `plannerAStarGrid` (8-connected grid A\*) + `plan` → packed `[meta; states]` + `shortenpath` greedy shortcut smoothing. **T3** `lidarScan` (Ranges/Angles ↔ Cartesian) + `matchScans` (point-to-point ICP via 2-D cross-covariance closed form) + `lidarSLAM` (`addScan` incremental scan-to-scan trajectory accumulation). **T4** `poseGraph` (SE2 nodes + relative-pose edges, `addRelativePose` chaining + loop-closure edges) + `optimizePoseGraph` (SE2-manifold Gauss-Newton over relative-pose residuals, node 1 fixed, recovers an exact unit square). Examples mirror the MathWorks UG pages: `nav_rotations.m` (Rotations/Orientation/Quaternions) · `nav_imu_intro.m` (Simulating IMU Measurements) · `nav_ground_vehicle.m` (Estimate Ground-Vehicle Pose, IMU+GPS → `insfilterMARG`) · `nav_rrt_plan.m` (Plan Mobile Robot Paths Using RRT). 6 gating tests. Carve-downs (`stateSpaceSE3`/Reeds-Shepp, `plannerHybridAStar`/`plannerBiRRT`, `factorGraph`/`poseGraph3D`, MCL/`stateEstimatorPF`/`controllerVFH`, GNSS/`gnssSensor`/Frenet/`insEKF`, MPNet/visual-SLAM/3-D point-cloud SLAM, ROS/Gazebo/Simulink) tracked in [`docs/navigation_toolbox_roadmap.md`](docs/navigation_toolbox_roadmap.md). |
