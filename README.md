...
[![Run-tests](https://img.shields.io/badge/run--tests-603_%E2%9C%93-2EA44F.svg)](test/Run)
...
| **Deep Learning** *(Tiers 1–3, dense)* | [`runtime/toolbox/dlnet/runtime_dlnet.cpp`](runtime/toolbox/dlnet/runtime_dlnet.cpp) + [`runtime/toolbox/dlnet/dlnet_classdefs.m`](runtime/toolbox/dlnet/dlnet_classdefs.m) | **T1 inference + T2 reverse-mode autodiff + T3 training shipped over the 2-D dense surface** — hand-coded over the shipped matrix kernel (no PyTorch/TF/cuDNN/ONNX-runtime). `dlarray` is a value carrier (data + tape-node id); a forward pass written with **natural operator overloading** (`W*X + b`) and activation/reduction/loss methods (`relu`/`sigmoid`/`tanh`/`softmax`/`sum`/`mean`/`log`/`exp`/`crossentropy`/`mse`) both **evaluates** (T1) and **records onto a thread-local reverse-mode tape** so **`dlgradient(loss, var)`** sweeps it (T2) — verified against central finite differences to **1.24e-10**; `extractdata` unwraps. **T3** closes the **custom training loop** end-to-end (`dl_mlp_train.m`: an MLP trained from scratch by SGD over the autodiff reaches 100% train accuracy, loss 1.80→0.01). The activation/op functions dispatch to the `dlarray` method only when an argument is `dlarray`-pinned (recursing through operators + ctor calls, so `relu(W*X+b)` is recognised via the pinned leaf `W`), leaving matrix `tanh`/`sum`/`log`/`exp` untouched. Examples `dl_mlp_infer.m` (forward) · `dl_autodiff_check.m` (gradient vs finite-diff) · `dl_mlp_train.m` (training). 3 gating tests. **Carve-downs**: convolution + 4-D `SSCB` tensors (no rank-N type yet — see [`any_shape_roadmap.md`](any_shape_roadmap.md) Tier C); the object-array `dlnetwork`/layer-object container (no classdef array literals — the functional/custom-loop form is the shipped surface); `dlgradient` returns the gradient matrix not a `dlarray`; the built-in `trainnet`/`trainingOptions` + functional solvers `adamupdate`/`sgdmupdate` (want `dlnetwork`/multi-return state). **T4–T6 + the DL HDL track** (recurrent/attention, architectures, tuning/quantization, fixed-point→SystemVerilog) tracked in [`docs/deep_learning_toolbox_roadmap.md`](docs/deep_learning_toolbox_roadmap.md). |
