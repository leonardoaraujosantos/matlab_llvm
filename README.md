...
[![Run-tests](https://img.shields.io/badge/run--tests-577_%E2%9C%93-2EA44F.svg)](test/Run)
...
| **Sensor Fusion and Tracking** *(Tiers 1–3 — inertial-nav half)* | [`runtime/toolbox/fusion/runtime_fusion.cpp`](runtime/toolbox/fusion/runtime_fusion.cpp) + [`runtime/toolbox/fusion/fusion_classdefs.m`](runtime/toolbox/fusion/fusion_classdefs.m) | **Tiers 1–3 shipped — the inertial-navigation half** — hand-coded over the shipped EKF/UKF cores from System Identification Tier-5 (no Eigen/GTSAM/Ceres). **T1** `quaternion` value type (N×4 [w x y z] storage) + Hamilton product / `conj`/`norm`/`normalize`/`inverse` / `slerp` / `dist` / `rotatepoint`/`rotateframe` / `quat2eul`/`eul2quat`/`quat2rotm`/`rotm2quat` + `ecompass` (Davenport's q-method) + new generic-math builtins `cross`/`dot`/`deg2rad`/`rad2deg`/`mvnrnd` (T1.7 core gaps). **T2** `trackingKF` (linear KF over `constvel`/`constacc`/`constturn` motion models + `cvmeas`/`cameas`/`ctmeas` measurement models) + `trackingEKF`/`trackingUKF` (vector-y re-skin of the shipped EKF/UKF cores via the function-handle ABI) + `objectDetection` container + `initcvekf`/`initctekf` initialisers. **T3** `imuSensor` (accel + gyro + optional mag, with bias / noise-density / random-walk params) + `gpsSensor` (lla + velocity) + `ahrsfilter`/`imufilter`/`complementaryFilter` (Mahony-style complementary orientation) + **`insfilterMARG` (headline)** (16-state quaternion + position + velocity + bias EKF fusing IMU + GPS) + `allanvar` noise analysis. Headlines `imu_gps_fusion.m` (the IMU+GPS dead-reckoning tracer) · `quaternion_rotations.m` · `tracking_ekf_2d.m`. 3 gating tests. Carve-downs (full insfilterMARG Joseph-form EKF, `tune`/`tunerconfig`, T4 trajectory generators, T5 multi-object trackers, T6 fusion+GOSPA metrics) tracked in [`docs/sensor_fusion_toolbox_roadmap.md`](docs/sensor_fusion_toolbox_roadmap.md). |
