...
[![Toolboxes](https://img.shields.io/badge/toolboxes-15_shipped-2EA44F.svg)](#shipped-toolboxes-in-the-runtime)
...
calls into. Thirteen toolbox surfaces ship today:
...
| **Image Processing** *(Tiers 1–3 cores of 6)* | [`runtime/toolbox/images/runtime_images.cpp`](runtime/toolbox/images/runtime_images.cpp) + [`runtime/toolbox/images/image_classdefs.m`](runtime/toolbox/images/image_classdefs.m) | **Tiers 1–3 cores shipped** — hand-coded over the shipped pixel substrate (no OpenCV/libpng/stb). Images are double matrices in [0,255] (grayscale M×N or slice-major M×N×3 RGB), reusing `conv2`/`imfilter`/`padarray`/`fft2`. **T1**: `imread`/`imwrite` (PGM/PPM/BMP) + `checkerboard`; `im2double`/`im2uint8`/`rgb2gray`/`mat2gray`; saturating arithmetic `imadd`/`imsubtract`/`imcomplement`/`imlincomb`; `imhist`/`imadjust`/`stretchlim`/`mean2`/`std2`. **T2**: `fspecial` (gaussian/sobel/laplacian/log/disk/…), `imgaussfilt`/`imboxfilt`/`medfilt2`/`ordfilt2`/`stdfilt`/`rangefilt`, and enhancement `histeq`/`adapthisteq`/`imsharpen`/`imhistmatch`/`imnoise`. **T3**: geometric — `imresize` (nearest/bilinear/bicubic), `imrotate` (crop/loose), `imcrop`, `imtranslate`, `imwarp` (affine + projective) with `affine2d`/`projective2d`/`imref2d` classdefs, and `fitgeotform2d` (LS affine/similarity from control points). Also added a general compiler fix (string-literal args auto-materialise to `matlab_string*` in the builtin dispatch). Headlines `examples/images/basic_image.m` + `filtering.m` + `geometric.m`. 3 gating tests. Tiers 4–6 (morphology, segmentation+`regionprops`, transforms) planned in [`docs/image_toolbox_roadmap.md`](docs/image_toolbox_roadmap.md). |
