...
| Signal Processing Toolbox (subset) | **Tier-1 lowpass design loop + Tier-2 bulk + Tier-3 (§4.1, §4.2, §4.3, §4.4) closed.** Tier-1: 17 windows; polynomial helpers (`roots`/`poly`/`polyder`/`polyint`/`residue`); IIR lowpass design (`butter`/`cheby1`/`cheby2` + `buttord`/`cheb1ord`); FIR (`fir1`/`sgolay`/`sgolayfilt`); filter impl (`filter`/`filtfilt`/`sosfilt`); response (`freqz`/`impz`/`stepz`/`grpdelay`). Tier-2: transforms (`dct`/`idct`/`fwht`/`hilbert`/`goertzel`); spectral (`periodogram`/`pwelch`/`cpsd`/`mscohere`/`tfestimate`); LP + parametric PSD (`levinson`/`lpc`/`aryule`/`arburg`/`pyulear`/`pburg`); time-frequency (`spectrogram`). Tier-3: real multirate (`upfirdn`/`decimate`/`interp`/`resample`), waveform generators (`chirp`/`sawtooth`/`square`/`gauspuls`/`rectpuls`/`tripuls`/`sinc`), alignment (`xcov`/`finddelay`/`dtw`), pulse measurements (`findpeaks`, `rms`/`peak2peak`/`peak2rms`/`rssq`, `medfilt1`/`hampel`/`envelope`, `midcross`/`risetime`/`falltime`/`dutycycle`). See [`docs/signal_toolbox_roadmap.md`](docs/signal_toolbox_roadmap.md). |
...
- `189` execution tests in `test/Run/` plus `4` opt-in symbolic tests in `test/RunSym/`
...
- [`docs/signal_toolbox_roadmap.md`](docs/signal_toolbox_roadmap.md): Signal Processing Toolbox compatibility plan — Tier-1 lowpass design loop, Tier-2 (nonparametric + parametric spectral, transforms tail, single-output spectrogram), and Tier-3 §4.1/§4.2/§4.3/§4.4 (real multirate, waveform generators, pulse measurements, alignment) are all closed (~80 functions across the C/C++/Python/TS lanes); still open are band variants of the IIR designers, `ellip`/`besself`, richer FIR (`fir2`/`firls`/`firpm`/`firrcos`/`kaiserord`), Gustafsson IC for `filtfilt`, multitaper (`dpss`/`pmtm`), STFT/`pspectrum`/`instfreq`/`instbw`, `czt`/`dst`/cepstrum, subspace AR methods, and the `digitalFilter` system object; explicit GUI / deep-learning / Simulink carve-outs documented
