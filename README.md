...
[![Run-tests](https://img.shields.io/badge/run--tests-606_%E2%9C%93-2EA44F.svg)](test/Run)
...
| **Deep Learning** *(Tiers 1–4: dense + LSTM + attention + embedding)* | [`runtime/toolbox/dlnet/runtime_dlnet.cpp`](runtime/toolbox/dlnet/runtime_dlnet.cpp) + [`runtime/toolbox/dlnet/dlnet_classdefs.m`](runtime/toolbox/dlnet/dlnet_classdefs.m) | **T1 inference + T2 reverse-mode autodiff + T3 training + T4 recurrent (LSTM) / attention / embedding shipped over the 2-D dense surface** — hand-coded over the shipped matrix kernel (no PyTorch/TF/cuDNN/ONNX-runtime). `dlarray` is a value carrier (data + tape-node id); a forward pass written with **natural operator overloading** (`W*X + b`) and activation/reduction/loss methods (`relu`/`sigmoid`/`tanh`/`softmax`/`sum`/`mean`/`log`/`exp`/`crossentropy`/`mse`) both **evaluates** (T1) and **records onto a thread-local reverse-mode tape** so **`dlgradient(loss, var)`** sweeps it (T2) — verified against central finite differences to **1.24e-10**; `extractdata` unwraps. **T3** closes the **custom training loop** (`dl_mlp_train.m`: MLP trained from scratch → 100% accuracy, loss 1.80→0.01). **T4 recurrent** ships **functional `lstm(X, H0, C0, W, R, b)`** as a single multi-parent tape node carrying every per-timestep gate + state — **BPTT for free** through `dlgradient` (`dl_lstm_sequence.m`: LSTM trained to remember the first bit of a 6-step sequence → 100%, loss 6→0). **T4 attention** adds `transpose` as an op so scaled-dot-product attention composes from existing ops (`dl_attention.m`: associative-recall task — the softmax peaks on the correct key, loss 1→0). **T4 embedding** ships `embed(E, idx)` — gather-forward + scatter-add-backward through `OP_EMBED`; repeated tokens correctly accumulate gradient (`dl_embed_train.m`: learns a 3×5 embedding table to per-element error < 0.01). Examples `dl_mlp_infer.m` · `dl_autodiff_check.m` · `dl_mlp_train.m` · `dl_lstm_sequence.m` · `dl_attention.m` · `dl_embed_train.m`. 6 gating tests. **Carve-downs**: convolution + 4-D `SSCB` tensors (no rank-N type yet — see [`any_shape_roadmap.md`](any_shape_roadmap.md) Tier C); object-array `dlnetwork`/layer container (no classdef array literals); `dlgradient` returns a matrix not a `dlarray`; built-in `trainnet`/`trainingOptions` + functional solvers `adamupdate`/`sgdmupdate`; GRU + bilstm (each is one extra opcode + pullback in the same custom-op pattern). **T5–T6 + the DL HDL track** (architectures, tuning/quantization, fixed-point→SystemVerilog) tracked in [`docs/deep_learning_toolbox_roadmap.md`](docs/deep_learning_toolbox_roadmap.md). |
