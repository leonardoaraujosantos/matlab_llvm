...
[![Run-tests](https://img.shields.io/badge/run--tests-602_%E2%9C%93-2EA44F.svg)](test/Run)
...
| **Deep Learning** *(Tiers 1–2, dense)* | [`runtime/toolbox/dlnet/runtime_dlnet.cpp`](runtime/toolbox/dlnet/runtime_dlnet.cpp) + [`runtime/toolbox/dlnet/dlnet_classdefs.m`](runtime/toolbox/dlnet/dlnet_classdefs.m) | **T1 inference + T2 reverse-mode autodiff shipped over the 2-D dense surface** — hand-coded over the shipped matrix kernel (no PyTorch/TF/cuDNN/ONNX-runtime). `dlarray` is a value carrier (data + tape-node id); a forward pass written with **natural operator overloading** (`W*X + b`) and activation/reduction/loss methods (`relu`/`sigmoid`/`tanh`/`softmax`/`sum`/`mean`/`log`/`exp`/`crossentropy`/`mse`) both **evaluates** (T1) and **records onto a thread-local reverse-mode tape** so **`dlgradient(loss, var)`** sweeps it (T2) — verified against central finite differences to **1.24e-10**; `extractdata` unwraps. The activation/op functions dispatch to the `dlarray` method only when an argument is `dlarray`-pinned (recursing through operators + ctor calls, so `relu(W*X+b)` is recognised via the pinned leaf `W`), leaving matrix `tanh`/`sum`/`log`/`exp` untouched. Examples `dl_mlp_infer.m` (2-layer softmax classifier forward) · `dl_autodiff_check.m` (gradient vs finite-diff). 2 gating tests. **Carve-downs**: convolution + 4-D `SSCB` tensors (no rank-N type yet — see [`any_shape_roadmap.md`](any_shape_roadmap.md) Tier C); the object-array `dlnetwork`/layer-object container (no classdef array literals — the functional/custom-loop form is the shipped surface); `dlgradient` returns the gradient matrix not a `dlarray`. **T3–T6 + the DL HDL track** (training solvers, recurrent/attention, architectures, tuning/quantization, fixed-point→SystemVerilog) are the documented follow-ons in [`docs/deep_learning_toolbox_roadmap.md`](docs/deep_learning_toolbox_roadmap.md) — T3 training now rests on the shipped autodiff keystone. |
