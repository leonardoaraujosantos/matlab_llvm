...
[![Run-tests](https://img.shields.io/badge/run--tests-650_%E2%9C%93-2EA44F.svg)](test/Run)
...
calls into. Twenty-three toolbox surfaces ship today:
...
| **Deep Learning** *(Tiers 1–6 + HDL H1–H4)* | [`runtime/toolbox/dlnet/runtime_dlnet.cpp`](runtime/toolbox/dlnet/runtime_dlnet.cpp) + [`runtime/toolbox/dlnet/dlnet_classdefs.m`](runtime/toolbox/dlnet/dlnet_classdefs.m) | **Tiers 1–4 shipped over the 2-D dense surface** — hand-coded over the shipped matrix kernel (no PyTorch/TF/cuDNN/ONNX-runtime). `dlarray` carries data + a tape-node id; the forward pass uses **natural operator overloading** (`W*X + b`) + activation/reduction/loss methods and **records onto a thread-local reverse-mode tape** so **`dlgradient(loss, var)`** sweeps it (T2; verified vs central finite differences to **1.24e-10**). **T3** custom training loop (`dl_mlp_train.m`: MLP → 100%, loss 1.80→0.01). **T4 recurrent**: functional **`lstm`** (`dl_lstm_sequence.m` first-bit-memory → 100%, loss 6→0); **`gru`** (same task → 100%, loss 6→0); **`bilstm`** (packed [forward; backward] weights, both directions BPTT'd in one tape node); **`lstmp`** (projected hidden, `dlgradient` accumulates `dP` alongside `dW/dR/db`). **T4 attention**: scaled-dot-product `V * softmax(transpose(K) * Q)` composes from existing ops via the added `transpose` (`dl_attention.m`: associative-recall — softmax peaks on the matching key, loss 1→0). **T4 embedding**: `embed(E, idx)` — gather-forward + scatter-add-backward; repeated tokens correctly accumulate (`dl_embed_train.m`: learns a 3×5 table to per-element error < 0.01). Activation/op dispatch fires *only* when an arg is `dlarray`-pinned (recursing through operators + ctor calls), so the matrix `tanh`/`sum`/`log`/`exp`/`transpose` lanes are unaffected. **T5 architectures / transfer / generative / metric-learning** ship as functional patterns: **residual blocks** via overloaded `plus` (`dl_residual_train.m`); **transfer learning** with frozen encoder as plain matrices + autodiff head (`dl_transfer_learn.m`: 96% on a 3-class task); **autoencoder** (`dl_autoencoder.m`: 4→2→4 compression, loss 0.45→0.08); **GAN** with LSGAN alternating losses (`dl_gan.m`: 1-D generator → N(2, 0.5²), D loss 4.0→0.3, gen mean 0.4→1.2); **Siamese** twin embedding with shared weights (`dl_siamese.m`: within-cluster ≈ 0 vs between-cluster ≈ 0.002). GAN + Siamese required the **classdef-operator-overloading dispatch fix** also landed here — `pinnedFromExpr` now recurses through dlarray-returning `CallOrIndex` on builtins (`mse`/`relu`/...), so binary `+` of two `mse` calls dispatches through `dlarray__plus` instead of silently lowering to `matlab_add_mm` and segfaulting. **15 gating tests**, 12 examples. **Carve-downs**: convolution + 4-D `SSCB` tensors (no rank-N type yet — see [`any_shape_roadmap.md`](any_shape_roadmap.md) Tier C); object-array `dlnetwork`/layer container (no classdef array literals — the functional/custom-loop form is the shipped surface); `dlgradient` returns a matrix not a `dlarray`; built-in `trainnet`/`trainingOptions` + functional solvers `adamupdate`/`sgdmupdate`; the `freezeLayers`/`replaceLayer`/`connectLayers` object-array APIs (the *patterns* they encapsulate ship as `dl_transfer_learn.m`'s custom training loop). **DL HDL Tier H1** (INT8 weight quantization) shipped — `dlquantize(W)` returns the dequantized weight matrix (rounded onto the int8 grid `[-127, 127]` per-tensor scaled at `max(abs(W))/127`); `dlqscale(W)` returns the scale.  Symmetric per-tensor quantization; software-side, no autodiff (post-training step). `dl_quantize_check.m`: trains the T3 MLP, INT8-quantizes every weight, re-runs inference — **both double and INT8 reach 100% accuracy** with max logit drift ≈ 0.1.  **DL HDL Tier H2** (fi-typed SystemVerilog emission) shipped — `examples/hdl/dlhdl_quant_mlp.m` is a Q16.8 2-2-1 quantized MLP forward written hand-unrolled in `fi` arithmetic; `matlabc -emit-sv` produces ~15 lines of synthesizable SystemVerilog (powers-of-2 weights fold to bit-shifts, ReLU as `z<0?0:z`, multi-layer linear) that passes **Verilator lint + Yosys synth** and joins the EmitSV regression sweep at **80/80 green**. **Tier 6 — tuning / metrics / interpretability / quantizer / verification (shipped):** classification metrics (`accuracy`/`precision`/`recall`/`fScore`/`rocmetrics`/`aucroc` reusing the Stats `confusionmat` kernel); **Grad-CAM-style attribution** via the autodiff (gradient of the picked-class logit w.r.t. inputs — `dl_gradcam.m` correctly points at the discriminating dimension for each class); **`dlqclip(X, scale)` + `dlqcalibrate(X, runningMaxAbs)`** for the activation-quantization calibration pass on top of H1's weight quantization; **Bayesian hyperparameter search** (`dl_bayesopt_hp.m` — wraps the shipped `bayesopt` on a synthetic loss-vs-learning-rate surface, locates the global minimum); **gradient-based l∞ robustness check** (`dl_robust_linf.m` — bounds the worst-case logit perturbation by ε·‖∇_x logit‖₁ via the dual-norm inequality; certifies the safe ε regime). 20 gating tests, 13 examples. **HDL H3 ✅ bit-accurate**: ships **two** new pragmas — `% cocotb: range(<port>, <lo>, <hi>)` (overflow-class divergence) and `% hdl: precise_fi` (FL-growth divergence; opt-in Sema-mono on the HW lane so matlab.matmul/add carry their fi-grown result types into SV emit). The dlhdl-MLP fixture now runs in the cocotb sweep at **41/41 with bit-accuracy** — SV emits 64-bit intermediates + the correctly-FL-rescaled bias (`64'sd8192` for 0.125 in Q34.16) and the Python reference agrees on every random vector. The 79 legacy EmitSV goldens (authored against Q16.8-throughout truncation) stay green — precise_fi is strict opt-in. Closes [issue #75](https://github.com/leonardoaraujosantos/matlab_llvm/issues/75). **HDL H4 ✅ LSTM-on-FPGA**: three precise_fi fixtures demonstrate the recurrent kernel compiles to bit-accurate fi SV — `dlhdl_rnn_cell.m` (simple cell `h_new = hardtanh(Wx*x + Wh*h_prev + b)`), `dlhdl_lstm_cell.m` (full LSTM with `hardsigmoid` i/f/o gates + `hardtanh` g gate + cell-state update + final hidden activation; combinational, caller threads h_prev/c_prev externally), and **`dlhdl_lstm_step.m`** (the **multi-timestep sequential variant**: same LSTM math but with `persistent` h_state / c_state registers that lower to the SV `always_comb` + `always_ff` shift-register pattern, so the recurrent loop closes *inside* the DUT — caller just clocks the module and streams x). All three pass cocotb at 100/100 — emitted SV uses 64-bit fi-grown intermediates with `sat_s64_b33`/`_b34` saturating helpers and explicit `>>> 64'sd8` Q-shifts at PWL clamp boundaries. cocotb sweep is now **44/44**. **Carved follow-ons**: `tsne` (T6.2), `dlnetwork` object-array surface, `trainnet`/`trainingOptions`, Neural ODE (T5.7) — all tracked in [`docs/deep_learning_toolbox_roadmap.md`](docs/deep_learning_toolbox_roadmap.md). |
