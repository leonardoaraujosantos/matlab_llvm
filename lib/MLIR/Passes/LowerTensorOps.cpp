...
bool TensorLowering::rewriteBuiltinCalls() {
  ...
  for (Operation *Call : Calls) {
    ...
    /* char([codes]): build a string from a vector/matrix of code points
     * (#234 symptom 3). Once the operand is a materialised matlab_mat (PtrTy)
     * — the matrix-literal concat_row lowers to one earlier in this fixpoint
     * pass, and a matrix variable is already PtrTy — route to the ready
     * matlab_char_m runtime entry. */
    if (Name == "char" && Call->getNumOperands() == 1 &&
        Call->getNumResults() == 1 && Call->getOperand(0).getType() == PtrTy) {
      B.setInsertionPoint(Call);
      auto Fn = rt("matlab_char_m", PtrTy, {PtrTy});
      auto NC = LLVM::CallOp::create(B, Call->getLoc(), Fn,
                                      ValueRange{Call->getOperand(0)});
      if (Call->getResult(0).getType() != PtrTy)
        Call->getResult(0).setType(PtrTy);
    ...
    static const Spec Table[] = {
      ...
      /* #189: row/column deletion A(i,:)=[] / A(:,j)=[]. Emitted directly by
      ...
      /* Number-theory vector / array forms (#235). primes / factor take a
       * scalar n and return a row vector; isprime/factorial here are the
       * matrix forms (scalar f64 args route to the _s Scalar map). The
       * numeric factor(n) reaches here only for a non-sym (f64) arg — the
       * symbolic factor(expr,var) is split off in the frontend (exprIsSym),
       * so the name collision with Symbolic Math is resolved by arg type. */
      {"primes",     "matlab_primes",     1, "f"},
      {"factor",     "matlab_factor",     1, "f"},
