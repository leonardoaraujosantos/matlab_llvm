...
std::string Emitter::name(mlir::Value V) {
  ...
  // LowerTensorOps (and any other pass that sets `matlab.name` on a
  // value's defining op). Falling back to a fresh `vN` only when no
  // hint is available keeps generated source readable for matrix
  // literals: `A = np.array(...)` instead of `v0 = np.array(...)`.
...
void Emitter::emitProlog() {
  ...
  // are added in the standalone `emitPython()` after we know what the
  // body actually references. Nothing else should depend on the prolog
...
void Emitter::emitFuncFunc(mlir::func::FuncOp F) {
  ...
  // → llvm.call sweep, so set calls survive as matlab.call_builtin.
...
void Emitter::emitOp(mlir::Operation &Op, int Indent) {
  ...
  // decl emitted alongside the function body. Other matlab.* op
  // shapes fall through to the unsupported diagnostic.
...
static bool bodyReferencesSymbol(llvm::StringRef Body,
  ...
  while ((I = Body.find(Prefix, I)) != llvm::StringRef::npos) {
    // Reject when the preceding character is part of an identifier — it
    // would mean we matched in the middle of a longer name like `art.x`.
    if (I > 0) {
      char C = Body[I - 1];
    ...
    // Reject matches inside a `# ...` comment line. Walk back to the
    // nearest newline; if we hit a `#` first, this match is in a comment.
    bool InComment = false;
    for (ssize_t J = (ssize_t)I - 1; J >= 0 && Body[J] != '\n'; --J) {
      if (Body[J] == '#') { InComment = true; break; }
    }
...
std::string emitPython(mlir::ModuleOp M, bool NoLine,
                       const matlab::SourceManager *SM) {
  /* Phase 6: the Python runtime ships no Symbolic Math Toolbox bridge
   * — it would route through SymPy but the emitter doesn't translate
   * the matlab_sym_* call shape to SymPy yet. Diagnose at the start
   * rather than silently emit code that fails at import. */
  bool HasSym = false;
  M.walk([&](mlir::Operation *Op) {
    if (HasSym) return;
    /* Pre-LLVM dialect: matlab.call_builtin carries the callee as a
     * StringAttr. Post-LLVM-conversion: llvm.func declarations carry
     * the runtime name as the symbol name. Check both so the
     * diagnostic fires regardless of where in the pipeline we run. */
  ...
  if (HasSym) {
    llvm::errs() << "error: -emit-python does not support the Symbolic "
                 << "Math Toolbox (matlab_sym_*) — use -emit-cpp / "
                 << "-emit-llvm or remove the symbolic operations\n";
    return {};
  }
  ...
  // Splice in only the imports the body actually references. Most
  // emitted programs use `rt.<helper>` for runtime calls; numpy is only
  // pulled in when a future emit path needs it (none today). Skipping
  // the unused imports keeps the generated source from claiming
  // dependencies it doesn't use.
  size_t SpliceAt = firstNewlineAfterHeader(Body);
  std::string Imports;
  if (bodyReferencesSymbol(Body, "rt."))
    Imports += "import matlab_runtime as rt\n";
  if (bodyReferencesSymbol(Body, "np."))
    Imports += "import numpy as np\n";
  // One blank line separates the header (and any imports) from the
  // body proper, keeping the file layout consistent regardless of
