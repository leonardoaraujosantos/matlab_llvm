...
| `interconnect_demo.m` | 2 ✅ | Full SISO interconnection workflow without model objects: `series_ss` (cascade), `parallel_ss` (sum), `feedback_ss` (closed-loop), `append_ss` (block-diagonal MIMO). Mass-spring-damper plant + integrator compensator; closed-loop verified Hurwitz. |
| `kalman_lqg.m` | 4.2 ✅ | Steady-state Kalman gain via `kalman_L` / `kalmd_L` + the LQG separation principle. 1×1 closed form `L = √2−1`, open-loop unstable plant Hurwitz-stabilised by the Kalman estimator, discrete Kalman gain on a Schur-stable plant, full LQR + Kalman side-by-side computation showing that LQG closed-loop poles are the union of the LQR closed-loop poles and the Kalman estimator poles. |
