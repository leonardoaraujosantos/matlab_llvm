...
| `quadrotor_pid_mpc_3d.m` | The same MPC + 4-PID cascade and 6-DOF RK4 plant, **visualised in 3-D** via the command-line [`sim3d`](../sim3d/) API: a body + four rotor hubs (parented to the body) record one keyframe per control step and export a self-contained Babylon.js player — watch the airframe tilt into the figure-8 turns. No plotting/FFmpeg deps; runs interpreted or compiled. Open the emitted `quadrotor_pid_mpc_3d.html` in a browser. |
