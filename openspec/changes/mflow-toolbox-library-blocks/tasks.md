...
- [x] 8.3 `signal_rl_agent` — trained policy-in-the-loop — DONE. An MLP policy (same W1/b1/W2/b2 form as dnn_predict) maps state → raw output; `actionType` post-processes — `discrete` emits the argmax action index (DQN-style), `continuous` emits `actionScale·tanh(raw)` (bounded, DDPG-style). `rl_agent.mflow` + SimulateRun (discrete argmax of [2 5 1] → 1; continuous saturating action bounded by ±actionScale).
